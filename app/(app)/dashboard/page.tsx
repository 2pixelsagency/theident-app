    ...
    const load = async () => {
      ...
      if (user) {
        ...
        if (profile) {
          ...
          setUserGenderId(profile.gender_id)
          setUserEthnicityId(profile.ethnicity_id)
          setUserHairColourId(profile.hair_colour_id)
          setUserEyeColourId(profile.eye_colour_id)
        }
  ...
  useEffect(() => {
    const scoreJob = (job: Job, jobSkills: Set<number>): ScoredJob => {
      ...
      if (job.gender_ids && job.gender_ids.length > 0) {
        if (userGenderId && job.gender_ids.includes(userGenderId)) {
          score += 5
      ...
      if (matchingSkillCount > 0) {
        const skillPoints = Math.min(matchingSkillCount * 3, 12)
        score += skillPoints
      ...
      if (job.ethnicity_ids && job.ethnicity_ids.length > 0) {
        if (userEthnicityId && job.ethnicity_ids.includes(userEthnicityId)) {
          score += 2
      ...
      if (job.hair_colour_ids && job.hair_colour_ids.length > 0) {
        if (userHairColourId && job.hair_colour_ids.includes(userHairColourId)) {
          score += 1
        }
      } else {
        score += 1
      ...
      if (job.eye_colour_ids && job.eye_colour_ids.length > 0) {
        if (userEyeColourId && job.eye_colour_ids.includes(userEyeColourId)) {
          score += 1
        }
      } else {
        score += 1
