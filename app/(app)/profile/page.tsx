    ...
    <div style={{ background: 'white', border: '1px solid #ebe8e1', borderRadius: '18px', padding: '14px 6px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ position: 'relative', width: size + 'px', height: size + 'px', marginBottom: '8px' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontFamily: "'ITC Symbol',Georgia,serif", letterSpacing: '-0.03em', fontSize: '17px', fontWeight: 500, color: '#0c2520' }}>{percent}%</span>
        </div>
      </div>
      <p style={{ fontSize: '11px', fontWeight: 500, color: '#0c2520', margin: '0 0 1px', textAlign: 'center' }}>{value}</p>
    ...
    <Link href="/calendar" style={{ textDecoration: 'none', flex: 1 }}>
      <div className="tap" style={{ background: 'white', border: '1px solid #ebe8e1', borderRadius: '16px', padding: '14px 12px', height: '100%' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: '2px', marginBottom: '4px' }}>
          {wk.map((w, i) => <span key={i} style={{ fontSize: '9px', color: '#bbb', textAlign: 'center', fontWeight: 500 }}>{w}</span>)}
        </div>
            ...
            return (
              <div key={i} style={{ height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                <div style={{ width: '21px', height: '21px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: isToday ? '#0c2520' : 'transparent' }}>
                  <span style={{ fontSize: '10px', fontWeight: isToday || hasEvent ? 500 : 400, color: isToday ? '#f1f0ee' : hasEvent ? '#0c2520' : '#bbb' }}>{d}</span>
                </div>
...
export default function Greenroom() {
  ...
  useEffect(() => { try { if (sessionStorage.getItem('ident_suggested_dismissed')) setSuggestedDismissed(true) } catch {} }, [])
  ...
  useEffect(() => {
    ...
    if (needsAction || weekEvents.length > 0 || viewsWeek > 0) setNudgeOpen(true)
  }, [loading])
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const { data: jobs } = await supabase.from('jobs').select('id, title, status, created_at').eq('created_by', user.id).order('created_at', { ascending: false }).limit(6)
        let list: PostedJob[] = (jobs || []).map((j: any) => ({ ...j, applicant_count: 0 }))
        try {
          const jobIds = list.map(j => j.id)
          if (jobIds.length) {
            const { data: apps } = await supabase.from('applications').select('job_id').in('job_id', jobIds)
            const cnt = new Map<string, number>()
            ;(apps || []).forEach((a: any) => cnt.set(a.job_id, (cnt.get(a.job_id) || 0) + 1))
            list = list.map(j => ({ ...j, applicant_count: cnt.get(j.id) || 0 }))
          }
        } catch {}
        setPostedJobs(list)
      } catch {}
  ...
  const dismissSuggested = () => { setSuggestedDismissed(true); try { sessionStorage.setItem('ident_suggested_dismissed', '1') } catch {} }
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', background: '#f1f0ee', minHeight: '100vh', paddingBottom: '100px' }}>
      <style>{`
        ...
        .overlay-fade { animation: overlayIn 0.25s ease-out; }
        .popup-fade { animation: popupIn 0.32s cubic-bezier(0.22,0.61,0.36,1); }
      ...
      {/* NUDGE POPUP */}
      {nudgeOpen && (
        <div className="overlay-fade" onClick={dismissNudge} style={{ position: 'fixed', inset: 0, background: 'rgba(6,20,16,0.4)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div className="popup-fade" onClick={e => e.stopPropagation()} style={{ background: focus.tone === 'dark' ? '#0c2520' : 'white', border: focus.tone === 'dark' ? 'none' : '1px solid #ebe8e1', borderRadius: '24px', padding: '28px 24px 24px', maxWidth: '320px', width: '100%', position: 'relative', textAlign: 'center' }}>
      ...
      {/* PROFILE VIEWS POPUP */}
      {viewsOpen && (
        <div className="overlay-fade" onClick={() => setViewsOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(6,20,16,0.4)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div className="popup-fade" onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '24px', padding: '24px', maxWidth: '360px', width: '100%', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}>
              <span style={{ fontSize: '10px', color: '#bbb' }}>1</span>
              <span style={{ fontSize: '10px', color: '#bbb' }}>{Math.ceil(daysInMonth / 2)}</span>
              <span style={{ fontSize: '10px', color: '#bbb' }}>{daysInMonth}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        {/* STREAK (top) */}
        <div className="stagger" style={{ animationDelay: '0.05s', background: 'white', border: '1px solid #ebe8e1', borderRadius: '18px', padding: '18px', marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', background: '#e6e2d9', borderRadius: '20px', padding: '3px' }}>
              {(['W', 'M', '6M'] as Period[]).map(p => (
                <button key={p} onClick={() => setPeriod(p)} className="tap" style={{ padding: '5px 11px', borderRadius: '16px', border: 'none', background: period === p ? '#0c2520' : 'transparent', color: period === p ? '#f1f0ee' : '#888', fontSize: '11px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>{p}</button>
              ))}
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
            <Link href="/profile/edit" className="tap" style={{ textDecoration: 'none' }}>
              <ProgressRing percent={completion} color="#0c2520" value="Profile" label="strength" />
            </Link>
            <div className="tap" onClick={() => setViewsOpen(true)} style={{ cursor: 'pointer' }}>
              <ProgressRing percent={viewsPct} color="#4ade80" value={viewsInPeriod + '/' + GOALS[period].views} label="views" />
            </div>
            <Link href="/applications" className="tap" style={{ textDecoration: 'none' }}>
              <ProgressRing percent={appsPct} color="#92d7af" value={appsInPeriod + '/' + GOALS[period].apps} label="applied" />
            </Link>
          </div>
        </div>

        {/* POSTED JOBS (prominent, casting) */}
        {postedJobs.length > 0 && (
                  ...
                  <Link key={j.id} href="/my-jobs" style={{ textDecoration: 'none' }}>
                    <div className="tap" style={{ background: 'white', border: '1px solid #ebe8e1', borderRadius: '16px', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: '14px', fontWeight: 500, color: '#0c2520', margin: '0 0 4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{j.title}</p>
                      ...
                      <div style={{ textAlign: 'center', background: j.applicant_count > 0 ? '#0c2520' : '#f3f0e9', borderRadius: '12px', padding: '8px 14px', minWidth: '64px', flexShrink: 0 }}>
          ...
          {connections.length > 0 ? (
            ...
            <Link href="/browse" style={{ textDecoration: 'none' }}>
              <div className="tap" style={{ background: '#0c2520', borderRadius: '16px', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <p style={{ fontFamily: "'ITC Symbol',Georgia,serif", letterSpacing: '-0.03em', fontSize: '16px', fontWeight: 500, color: '#f1f0ee', margin: '0 0 4px' }}>Build your network</p>
                    ...
                    return (
                      <Link key={e.id} href="/calendar" style={{ textDecoration: 'none' }}>
                        <div style={{ display: 'flex', gap: '8px', padding: '8px 0', borderBottom: i < Math.min(weekEvents.length, 4) - 1 ? '1px solid #f3f0e9' : 'none' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ fontSize: '12px', fontWeight: 500, color: '#0c2520', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.title}</p>
        ...
        {myCommunities.length > 0 && (
          <div className="stagger" style={{ animationDelay: '0.25s' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <p style={{ fontSize: '11px', color: '#999', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600, margin: 0 }}>Your communities</p>
                ...
                <Link key={c.id} href={'/communities/' + c.slug} style={{ textDecoration: 'none', flexShrink: 0 }}>
                  <div className="tap" style={{ width: '150px', background: 'white', borderRadius: '14px', border: '1px solid #ebe8e1', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '8px 12px 12px' }}>
                      ...
                      <p style={{ fontSize: '13px', fontWeight: 500, color: '#0c2520', margin: '0 0 2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</p>
        ...
        {/* SUGGESTED (after communities, dismissible) */}
        {!suggestedDismissed && suggested.length > 0 && (
          <div className="stagger" style={{ animationDelay: '0.3s' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <p style={{ fontSize: '11px', color: '#999', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600, margin: 0 }}>Suggested for you</p>
              <button onClick={dismissSuggested} aria-label="Dismiss" style={{ width: '26px', height: '26px', borderRadius: '50%', background: '#e8e4de', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2.4" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
              </button>
            </div>
            <div className="scroll-row" style={{ display: 'flex', gap: '10px', overflowX: 'auto', marginBottom: '24px', paddingBottom: '4px' }}>
              {suggested.map(c => (
