...
// Application stages in order — "auditioned" = audition and beyond
...
function ProgressRing({ percent, color, main, label, onDismiss }: { percent: number; color: string; main: string; label: string; onDismiss?: () => void }) {
  ...
  return (
    <div style={{ position: 'relative', scrollSnapAlign: 'start', flexShrink: 0, width: '108px', background: 'white', border: '1px solid #ebe8e1', borderRadius: '18px', padding: '14px 6px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      {onDismiss && (
        <button onClick={(e) => { e.stopPropagation(); onDismiss() }} style={{ position: 'absolute', top: '7px', right: '7px', width: '20px', height: '20px', borderRadius: '50%', background: '#f3f0e9', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', padding: 0 }}>
...
export default function Greenroom() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // communities (merge joined + suggested)
      ...
      // Phase 2 — derived counts in parallel
  ...
  const rings = [
    { key: 'profile_strength', color: '#0c2520', main: completion + '%', label: 'profile', pct: completion, dismissible: completion >= 100 },
