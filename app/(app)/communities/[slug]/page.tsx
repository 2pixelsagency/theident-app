...
export default function CommunityDetail() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'cover' | 'icon') => {
    ...
    if (!error) {
      ...
      const update = type === 'cover' ? { cover_url: publicUrl } : { icon_url: publicUrl }
      await supabase.from('communities').update(update).eq('id', community.id)
      setCommunity({ ...community, ...update })
      showToast(type === 'cover' ? 'Cover updated' : 'Profile picture updated')
    }
    setUploading(false)
    e.target.value = ''
  }
  ...
  const handlePost = async () => {
    ...
    setNewPost(''); setPostCatId(null); setEventDate(''); setEventTime(''); setEventLocation('')
    setPosting(false)
  }
  ...
  const togglePin = async (postId: string) => {
    ...
    setPosts(posts.map(p => p.id === postId ? { ...p, is_pinned: !p.is_pinned } : p).sort((a, b) => (b.is_pinned ? 1 : 0) - (a.is_pinned ? 1 : 0)))
    showToast(post.is_pinned ? 'Unpinned' : 'Pinned')
  }
  ...
  const handleVote = async (postId: string, voteType: 'up' | 'down') => {
    ...
    if (post.my_vote === voteType) {
      await supabase.from('community_post_likes').delete().eq('post_id', postId).eq('profile_id', userId)
      setPosts(posts.map(p => p.id === postId ? { ...p, my_vote: null, upvotes: voteType === 'up' ? p.upvotes - 1 : p.upvotes, downvotes: voteType === 'down' ? p.downvotes - 1 : p.downvotes } : p))
    } else {
      if (post.my_vote) {
        await supabase.from('community_post_likes').delete().eq('post_id', postId).eq('profile_id', userId)
      }
      await supabase.from('community_post_likes').insert({ post_id: postId, profile_id: userId, vote_type: voteType })
      setPosts(posts.map(p => {
        if (p.id !== postId) return p
        let ups = p.upvotes; let downs = p.downvotes
        if (p.my_vote === 'up') ups--; if (p.my_vote === 'down') downs--
        if (voteType === 'up') ups++; if (voteType === 'down') downs++
        return { ...p, my_vote: voteType, upvotes: ups, downvotes: downs }
      }))
    }
  }
  ...
  const handleReply = async (postId: string) => {
    ...
    if (data) {
      ...
      const s = new Set(expandedReplies); s.add(postId); setExpandedReplies(s)
    }
  ...
  const addCategory = async () => {
    if (!community || !newCatName.trim()) return
    const { data } = await supabase.from('community_categories').insert({ community_id: community.id, name: newCatName, sort_order: categories.length }).select().single()
    if (data) setCategories([...categories, data])
    setNewCatName(''); setShowAddCat(false); showToast('Category added')
  }

  const deleteCategory = async (catId: string) => {
    await supabase.from('community_categories').delete().eq('id', catId)
  ...
  const filteredPosts = feedFilter ? posts.filter(p => p.category_id === feedFilter) : posts
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'end', gap: '14px', marginTop: '-30px', position: 'relative', zIndex: 2, marginBottom: '12px' }}>
          <div style={{ position: 'relative' }}>
            ...
            {isOwner && (
              <button onClick={() => iconRef.current?.click()} style={{ position: 'absolute', bottom: '-2px', right: '-2px', width: '22px', height: '22px', borderRadius: '50%', background: '#0c2520', border: '2px solid #f1f0ee', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
          ...
          <div style={{ paddingBottom: '4px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <p style={{ fontFamily: "'ITC Symbol',Georgia,serif", letterSpacing: '-0.03em', fontSize: '22px', fontWeight: 700, color: '#0c2520', margin: 0 }}>{community.name}</p>
              {community.is_private && <div style={{ background: '#0c2520', borderRadius: '50%', width: '18px', height: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg></div>}
            </div>
            <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', color: '#888' }}>{approvedMembers.length} members</span>
              <span style={{ fontSize: '10px', color: '#888', background: '#e8e4de', padding: '2px 8px', borderRadius: '4px' }}>{community.category}</span>
                  ...
                  <div style={{ padding: '14px 16px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', background: '#f9f8f6', borderRadius: '20px', overflow: 'hidden' }}>
                        <button onClick={() => handleVote(p.id, 'up')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '6px 10px', display: 'flex', alignItems: 'center', gap: '3px', fontSize: '12px', color: p.my_vote === 'up' ? '#4ade80' : '#888', fontFamily: 'inherit' }}>
                          <svg width="14" height="14" viewBox="0 0 24 24" fill={p.my_vote === 'up' ? '#4ade80' : 'none'} stroke={p.my_vote === 'up' ? '#4ade80' : '#888'} strokeWidth="2" strokeLinecap="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg>
                          {p.upvotes > 0 && p.upvotes}
                        </button>
                        <div style={{ width: '1px', height: '16px', background: '#e0ddd5' }} />
                        <button onClick={() => handleVote(p.id, 'down')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '6px 10px', display: 'flex', alignItems: 'center', gap: '3px', fontSize: '12px', color: p.my_vote === 'down' ? '#c0392b' : '#888', fontFamily: 'inherit' }}>
                          <svg width="14" height="14" viewBox="0 0 24 24" fill={p.my_vote === 'down' ? '#c0392b' : 'none'} stroke={p.my_vote === 'down' ? '#c0392b' : '#888'} strokeWidth="2" strokeLinecap="round"><path d="M12 5v14M5 12l7 7 7-7"/></svg>
                          {p.downvotes > 0 && p.downvotes}
                        </button>
                      </div>
          ...
          <div>
            ...
            {isOwner && (
              <div style={{ background: 'white', borderRadius: '14px', border: '1px solid #e8e4de', padding: '16px', marginBottom: '12px' }}>
                ...
                {categories.map(c => (
                  <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #f0ede5' }}>
                    <span style={{ fontSize: '13px', color: '#0c2520' }}>{c.name}</span>
                    <button onClick={() => deleteCategory(c.id)} style={{ background: 'none', border: 'none', fontSize: '11px', color: '#c0392b', cursor: 'pointer', fontFamily: 'inherit' }}>Remove</button>
                  </div>
                ))}
                {showAddCat ? (
                  <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                    <input value={newCatName} onChange={e => setNewCatName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') addCategory() }} placeholder="Category name" style={{ flex: 1, padding: '8px 12px', border: '1px solid #e0ddd5', borderRadius: '8px', fontSize: '13px', fontFamily: 'inherit', color: '#0c2520', outline: 'none' }} />
                    <button onClick={addCategory} style={{ padding: '8px 14px', background: '#0c2520', color: '#f1f0ee', border: 'none', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit' }}>Add</button>
                  </div>
                ) : (
