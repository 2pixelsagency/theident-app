...
export default function PostJob() {
  ...
  const [applicationMethod, setApplicationMethod] = useState('in_app')
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setProductionTypes(pt.data || []); setGenders(g.data || []); setEthnicities(e.data || [])
      setHairColours(h.data || []); setEyeColours(ey.data || []); setSkills(s.data || []); setSkillCategories(sc.data || [])
    }
    load()
  }, [])
  ...
  const filteredSkills = skillSearch ? skills.filter(s => s.name.toLowerCase().includes(skillSearch.toLowerCase())).slice(0, 8) : []
  ...
  const handleSubmit = async () => {
    ...
    const baseData = {
      created_by: user.id, is_side_hustle: isSideHustle, is_published: true, location, salary, description,
      casting_email: isSideHustle ? applyEmail : castingEmail,
      application_method: applicationMethod,
      application_email: applicationMethod === 'email' ? (isSideHustle ? applyEmail : castingEmail) : null,
      requires_nda: requiresNda,
      nda_text: requiresNda ? ndaText : null,
    }
    const jobData: Record<string, unknown> = isSideHustle
      ? { ...baseData, ...spotlightFields, job_title: jobTitle, company, job_category: jobCategory, commitment_level: commitmentLevel, experience_level: experience, audition_friendly: auditionFriendly, schedule, dbs_required: dbsRequired, start_date: startDate || null }
      : { ...baseData, ...spotlightFields, project_role: projectRole, project_in: projectIn, production_company: productionCompany, short_summary: shortSummary, production_type_id: productionTypeId, contract_dates: contractDates, application_deadline: applicationDeadline || null, age_range: minAge + '-' + maxAge, gender_ids: selectedGenders, ethnicity_ids: selectedEthnicities, hair_colour_ids: selectedHairColours, eye_colour_ids: selectedEyeColours, appearance_notes: appearanceNotes, height, casting_team: castingTeam, submission_link: submissionLink }
    const { data: newJob, error } = await supabase.from('jobs').insert(jobData).select().single()
    if (error) { alert('Failed to post job: ' + error.message); setSaving(false); return }
    ...
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: hint ? '6px' : 0 }}>
        {items.map(item => {
          ...
          return <button key={item.id} type="button" onClick={() => toggleInArray(selected, setSelected, item.id)} style={{ padding: '6px 12px', border: isSelected ? '1.5px solid #0c2520' : '1px solid #e0ddd5', borderRadius: '8px', background: isSelected ? '#e8efea' : 'white', cursor: 'pointer', fontSize: '13px', color: '#0c2520', fontFamily: 'inherit', fontWeight: isSelected ? 500 : 400, transition: 'all 0.15s ease' }}>{item.name}</button>
        })}
      </div>
  ...
  return (
    <div>
      <style>{`
        ...
        .salary-pill { padding: 8px 14px; border-radius: 8px; font-size: 13px; cursor: pointer; transition: all 0.2s ease; font-family: inherit; }
        ...
        .range-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 16px; height: 16px; background: #0c2520; border-radius: 50%; cursor: pointer; border: none; }
        .range-slider::-moz-range-thumb { width: 16px; height: 16px; background: #0c2520; border-radius: 50%; cursor: pointer; border: none; }
        ...
        {!isSideHustle && (
          <div style={{ background: 'white', border: '1px solid #e8e6e0', borderRadius: '12px', padding: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div><label style={labelStyle}>Role <span style={{ color: '#c44' }}>*</span></label><input type="text" value={projectRole} onChange={e => setProjectRole(e.target.value)} placeholder="e.g. Charlie" style={inputStyle} /></div>
              <div><label style={labelStyle}>Project Name <span style={{ color: '#c44' }}>*</span></label><input type="text" value={projectIn} onChange={e => setProjectIn(e.target.value)} placeholder="e.g. To Make Ends Meat" style={inputStyle} /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div><label style={labelStyle}>Production Company <span style={{ color: '#c44' }}>*</span></label><input type="text" value={productionCompany} onChange={e => setProductionCompany(e.target.value)} placeholder="e.g. BBC" style={inputStyle} /></div>
              <div><label style={labelStyle}>Production Type</label><select value={productionTypeId || ''} onChange={e => setProductionTypeId(e.target.value ? Number(e.target.value) : null)} style={inputStyle}><option value="">Select...</option>{productionTypes.map(pt => <option key={pt.id} value={pt.id}>{pt.name}</option>)}</select></div>
            </div>
            <div style={{ marginBottom: '12px' }}><label style={labelStyle}>One-line Summary <span style={{ color: '#c44' }}>*</span></label><input type="text" value={shortSummary} onChange={e => setShortSummary(e.target.value)} placeholder="What's this role about in one sentence?" style={inputStyle} /></div>
            <div>
              <label style={labelStyle}>Payment <span style={{ color: '#c44' }}>*</span></label>
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '8px' }}>
                {['Paid', 'Equity', 'Expenses Only', 'Profit Share'].map(opt => <button key={opt} type="button" onClick={() => setSalaryType(opt)} className="salary-pill" style={{ background: salaryType === opt ? '#e8efea' : 'white', border: salaryType === opt ? '1.5px solid #0c2520' : '1px solid #e0ddd5', color: '#0c2520', fontWeight: salaryType === opt ? 500 : 400 }}>{opt}</button>)}
              </div>
              {salaryType === 'Paid' && <input type="text" value={salaryAmount} onChange={e => setSalaryAmount(e.target.value)} placeholder="e.g. £500/day or £2000 total" style={inputStyle} />}
            </div>

            <h2 style={sectionStyle}>When and where</h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div><label style={labelStyle}>Location</label><input type="text" value={location} onChange={e => setLocation(e.target.value)} placeholder="e.g. London, Cardiff" style={inputStyle} /></div>
              <div><label style={labelStyle}>Contract Dates</label><input type="text" value={contractDates} onChange={e => setContractDates(e.target.value)} placeholder="e.g. June 15-25 2026" style={inputStyle} /></div>
            </div>
            <div><label style={labelStyle}>Application Deadline</label><input type="date" value={applicationDeadline} onChange={e => setApplicationDeadline(e.target.value)} style={inputStyle} /></div>
            ...
            <div style={{ marginBottom: '20px', position: 'relative' }}>
              ...
              {filteredSkills.length > 0 && (
                <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: 'white', border: '1px solid #e0ddd5', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)', maxHeight: '200px', overflowY: 'auto', zIndex: 10 }}>
                  {filteredSkills.map(skill => <button key={skill.id} type="button" onClick={() => { toggleInArray(selectedSkills, setSelectedSkills, skill.id); setSkillSearch('') }} style={{ width: '100%', textAlign: 'left', padding: '10px 14px', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '13px', color: '#0c2520', fontFamily: 'inherit', borderBottom: '1px solid #f0f0f0' }}>{skill.name}</button>)}
                </div>
              )}
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div><label style={{ fontSize: '11px', color: '#888', marginBottom: '4px', display: 'block' }}>Min</label><input type="range" min="0" max="100" value={minAge} onChange={e => { const v = Number(e.target.value); if (v <= maxAge) setMinAge(v) }} className="range-slider" /></div>
                <div><label style={{ fontSize: '11px', color: '#888', marginBottom: '4px', display: 'block' }}>Max</label><input type="range" min="0" max="100" value={maxAge} onChange={e => { const v = Number(e.target.value); if (v >= minAge) setMaxAge(v) }} className="range-slider" /></div>
              </div>
            </div>

            <div style={{ marginBottom: '20px' }}><label style={labelStyle}>Gender (select all that apply)</label><MultiSelectPills items={genders} selected={selectedGenders} setSelected={setSelectedGenders} hint="Leave empty for any gender" /></div>
            <div style={{ marginBottom: '20px' }}><label style={labelStyle}>Ethnicity (select all that apply)</label><MultiSelectPills items={ethnicities} selected={selectedEthnicities} setSelected={setSelectedEthnicities} hint="Leave empty for any ethnicity" /></div>
            <div style={{ marginBottom: '20px' }}><label style={labelStyle}>Hair Colour</label><MultiSelectPills items={hairColours} selected={selectedHairColours} setSelected={setSelectedHairColours} hint="Leave empty for any hair colour" /></div>
            <div style={{ marginBottom: '20px' }}><label style={labelStyle}>Eye Colour</label><MultiSelectPills items={eyeColours} selected={selectedEyeColours} setSelected={setSelectedEyeColours} hint="Leave empty for any eye colour" /></div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
              <div><label style={labelStyle}>Other Appearance Notes</label><input type="text" value={appearanceNotes} onChange={e => setAppearanceNotes(e.target.value)} placeholder="e.g. tattoos welcome, athletic build" style={inputStyle} /></div>
              <div><label style={labelStyle}>Height (optional)</label><div style={{ position: 'relative' }}><input type="number" value={height} onChange={e => setHeight(e.target.value)} placeholder="170" style={{ ...inputStyle, paddingRight: '50px' }} /><span style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', color: '#999', fontSize: '13px', pointerEvents: 'none' }}>cm</span></div></div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div><label style={labelStyle}>Casting Email <span style={{ color: '#c44' }}>*</span></label><input type="email" value={castingEmail} onChange={e => setCastingEmail(e.target.value)} placeholder="castings@hbcastings.co.uk" style={inputStyle} /></div>
              <div><label style={labelStyle}>Casting Director / Team</label><input type="text" value={castingTeam} onChange={e => setCastingTeam(e.target.value)} placeholder="e.g. Heather Basten CDG" style={inputStyle} /></div>
            </div>
            <div style={{ marginBottom: '12px' }}><label style={labelStyle}>Submission Link (optional)</label><input type="text" value={submissionLink} onChange={e => setSubmissionLink(e.target.value)} placeholder="Dropbox, Google Drive, etc." style={inputStyle} /></div>
            <div style={{ marginBottom: '24px' }}><label style={labelStyle}>Full Description <span style={{ color: '#c44' }}>*</span></label><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe the role, character, and what you're looking for..." rows={6} style={{ ...inputStyle, resize: 'vertical' }} /></div>
        ...
        {isSideHustle && (
          <div style={{ background: 'white', border: '1px solid #e8e6e0', borderRadius: '12px', padding: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              ...
              <div><label style={labelStyle}>Company <span style={{ color: '#c44' }}>*</span></label><input type="text" value={company} onChange={e => setCompany(e.target.value)} placeholder="e.g. Frame Studios" style={inputStyle} /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div><label style={labelStyle}>Category</label><select value={jobCategory} onChange={e => setJobCategory(e.target.value)} style={inputStyle}><option value="">Select...</option><option value="Fitness">Fitness & Wellness</option><option value="Teaching">Teaching</option><option value="Hospitality">Hospitality</option><option value="Creative Arts">Creative Arts</option><option value="Childcare">Childcare</option><option value="Admin">Admin / Office</option><option value="Retail">Retail</option><option value="Other">Other</option></select></div>
              <div><label style={labelStyle}>Commitment Level</label><select value={commitmentLevel} onChange={e => setCommitmentLevel(e.target.value)} style={inputStyle}><option value="">Select...</option><option value="One-off">One-off</option><option value="Casual">Casual</option><option value="Part-time">Part-time</option><option value="Full-time">Full-time</option></select></div>
            </div>

            <h2 style={sectionStyle}>Details</h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div><label style={labelStyle}>Experience Required</label><input type="text" value={experience} onChange={e => setExperience(e.target.value)} placeholder="e.g. 2 years teaching" style={inputStyle} /></div>
              <div><label style={labelStyle}>Schedule</label><input type="text" value={schedule} onChange={e => setSchedule(e.target.value)} placeholder="e.g. Mon-Fri evenings" style={inputStyle} /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div><label style={labelStyle}>Location</label><input type="text" value={location} onChange={e => setLocation(e.target.value)} placeholder="e.g. Shoreditch, London" style={inputStyle} /></div>
              <div><label style={labelStyle}>Salary / Rate</label><input type="text" value={salaryAmount} onChange={e => setSalaryAmount(e.target.value)} placeholder="e.g. £25/class" style={inputStyle} /></div>
            </div>
            <div style={{ marginBottom: '16px' }}><label style={labelStyle}>Start Date</label><input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} style={inputStyle} /></div>
            <div style={{ display: 'flex', gap: '20px', marginBottom: '20px', flexWrap: 'wrap' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#0c2520', cursor: 'pointer' }}><input type="checkbox" checked={auditionFriendly} onChange={e => setAuditionFriendly(e.target.checked)} style={{ accentColor: '#0c2520', width: '14px', height: '14px' }} />Audition Friendly</label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#0c2520', cursor: 'pointer' }}><input type="checkbox" checked={dbsRequired} onChange={e => setDbsRequired(e.target.checked)} style={{ accentColor: '#0c2520', width: '14px', height: '14px' }} />DBS Required</label>
            </div>

            <h2 style={sectionStyle}>How to apply</h2>
            <ApplicationMethodSelector />

            <div style={{ marginBottom: '12px' }}><label style={labelStyle}>Apply Email <span style={{ color: '#c44' }}>*</span></label><input type="email" value={applyEmail} onChange={e => setApplyEmail(e.target.value)} placeholder="hello@yourcompany.com" style={inputStyle} /></div>
            <div style={{ marginBottom: '24px' }}><label style={labelStyle}>Description <span style={{ color: '#c44' }}>*</span></label><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe the role, what you're looking for, perks..." rows={6} style={{ ...inputStyle, resize: 'vertical' }} /></div>
        ...
        {showPayment && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(12, 37, 32, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
            <div style={{ background: 'white', borderRadius: '16px', padding: '28px', maxWidth: '420px', width: '100%' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
                <div><label style={labelStyle}>Expiry</label><input type="text" value={cardExpiry} onChange={e => setCardExpiry(e.target.value)} placeholder="MM/YY" style={inputStyle} /></div>
                <div><label style={labelStyle}>CVC</label><input type="text" value={cardCvc} onChange={e => setCardCvc(e.target.value)} placeholder="123" style={inputStyle} /></div>
              </div>
              ...
              <button onClick={() => { setShowPayment(false); setSpotlightJob(false) }} style={{ width: '100%', background: 'transparent', color: '#666', border: 'none', padding: '8px', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>Cancel & Post Without Spotlight</button>
            </div>
          </div>
        )}
      </div>
